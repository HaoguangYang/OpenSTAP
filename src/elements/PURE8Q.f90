...
! N107: NLAST
  ...
  CALL PURE8 (IA(NP(1)),DA(NP(2)),DA(NP(3)),DA(NP(4)),DA(NP(4)),IA(NP(5)),   &
       A(N(1)),A(N(2)),A(N(3)),A(N(4)),A(N(5)),A(N(6)),A(N(7)))

  RETURN

END SUBROUTINE PURE8Q


SUBROUTINE PURE8 (ID,X,Y,Z,U,MHT,E,POSSION,LM,XYZ,MATP, THICK, Node)
  USE GLOBALS
  USE MEMALLOCATE
  USE MathKernel
  ...
  REAL(8) :: G1, G2, GN(2,4), GN8(2,8), Ja(2,2), Ja_inv(2,2), S(16,16), BB(2,8), NN0(1,8)
  REAL(8) :: X_Y(4, 2), STR1(3), STR2(2), Bm(3,16), GaussianCollection(3, NPAR(2)*9)
  NPAR1  = NPAR(1)
  NUME   = NPAR(2)
  NUMMAT = NPAR(3) 
...
!       Save element information
        ...
        DO M=1,2
           LM(M:M+15:2,N)=ID(M,Node(N,:))     ! Connectivity matrix
        END DO
...
! 为平面部分的Bm赋值。改成循环
                ...
                END DO
 ! 这里不要忘了还要乘上z方向积分
                S = S + (matmul(matmul(transpose(Bm), Cm), Bm))*abs(detJ)*GAUSS_COF(L)*GAUSS_COF(M)
...
! 为平面部分的Bm赋值。改成循环
            ...
            STR1 = reshape( matmul(Cm,matmul(Bm,DE)),(/3/))
            STR2(2:1:-1) = 0
